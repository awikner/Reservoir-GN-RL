...
"""  # here starts the code: (beating the code folding glitch)
    ...
    def ask(self, number=None, xmean=None, sigma_fac=1,
        ...
        phenomean = self.gp.pheno(self.mean, copy = True, into_bounds = self.boundary_handler.repair)
        
        f = open('%sphenomean.txt' % self.opts['verb_filenameprefix'], 'a')
        f.write('%e' % phenomean[0])
        for i in range(1, phenomean.size):
            f.write(', %e' % phenomean[i])
        f.write('\n')
        f.close
    ...
    Variances `self.sm.variances`=%s""" % str(self.sm.variances))
        ...
        f = open('%scovmats.txt' % self.opts['verb_filenameprefix'], 'a')
        Cflat = self.sm.covariance_matrix.flatten()
        f.write('%e' % Cflat[0])
        for i in range(1, Cflat.size):
            f.write(', %e' % Cflat[i])
        f.write('\n')
        f.close
