...
class reservoir:
    ...
    def trainWoutRLS(self, train_data, sync_length):
        self.r, self.Wout, self.data_trstates, self.states_trstates_inv = trainWoutRLS(train_data, sync_length, \
                 self.forget, self.A, self.Win, self.leakage, self.LM_regularization, self.LM_t_regularization, \
                 self.regularization, self.t_regularization, self.delta, self.timestep, self.RLS_reg_type, self.t_weighted)
...
def vt_min_function_rls(data, hyperparams, mask, base_Win, base_A, num_nodes = 210, \
    ...
    for i in range(len(mask)):
        if mask[i] == 'input_weight':
            input_weight = 0.5*(hyperparams[i]-5.)
...
def cross_validation_performance_separated(data, reservoir, num_tests, sync_length, train_length, pred_length, \
                                 errormax = 3.2, train_method = 'RLS', progress = False):
    # Evaluates the valid time of prediction over a random set of training time series and test time series
    data_length = data.shape[0]
