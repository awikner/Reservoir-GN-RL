...
class reservoir:
    ...
    def getTargetFeatureArray(self, aug_states, train_data, pass_inputs, pass_external, sync_length):
        ...
        if len(pass_inputs) == 1:
            pass_input_data = train_data[sync_length-1:train_data.shape[0]-1,pass_inputs].reshape(-1,1)
            feature_array    = np.hstack((feature_array, pass_input_data))
        elif len(pass_inputs) > 1:
            pass_input_data = train_data[sync_length-1:train_data.shape[0]-1,pass_inputs]
            feature_array    = np.hstack((feature_array, pass_input_data))
        if pass_external.size > 0 and self.external_output:
            pass_external_data = pass_external[sync_length:]
    ...
    def trainWout(self, train_data, sync_length, pass_data = [], external_input = True, external_output = False):
        ...
        if isinstance(pass_data, tuple):
            ...
            pass_inputs = pass_data
            pass_external = np.array([])
        elif isinstance(pass_data, np.ndarray):
            pass_inputs = []
            pass_external = pass_data
        if external_input and not pass_external.size == 0:
            input_data = np.hstack((train_data, pass_external))
        else:
        ...
        """
        pass_input_data = train_data[sync_length-1:train_data.shape[0]-1,pass_inputs]
        if len(pass_input_data.shape)==1:
            pass_input_data = pass_input_data.reshape(-1,1)
        if pass_external.size == 0:
            pass_external_data = pass_external
        else:
            pass_external_data = pass_external[sync_length:]
            if len(pass_external_data.shape)==1:
                pass_external_data = pass_external_data.reshape(-1,1)
        print(aug_states.shape)
        print(pass_input_data.shape)
        print(pass_external_data.shape)
        print(train_data[sync_length:].shape)
        """
    ...
    def predict(self, pass_inputs, pass_external):
        ...
        aug_r[::2] = np.power(aug_r[::2],2)
        if len(pass_inputs) == 1:
            aug_r = np.append(aug_r, np.array(self.last_input[pass_inputs]))
        elif len(pass_inputs) > 1:
            aug_r = np.append(aug_r, self.last_input[pass_inputs])
        if pass_external.size > 0 and self.external_output:
            aug_r = np.append(aug_r, pass_external)
    ...
    def valid_time(self, validation_data, pass_data = [], error_bound = 0.2, plot = False):
        ...
        if isinstance(pass_data, tuple):
            ...
            pass_inputs = []
            pass_external = pass_data
        if len(pass_external.shape) == 1:
            pass_external = pass_external.reshape(-1,1)
        valid_time = 0
        predictions = np.zeros(validation_data.shape)
        if self.external_input and pass_external.size > 0:
            predictions = np.hstack((predictions, pass_external))
        errors = np.zeros(validation_data.shape[0])
        initial_r = np.copy(self.r)
        if not self.t_weighted:
            for i in range(validation_data.shape[0]):
                if self.external_input and pass_external.size > 0:
                    predictions[i,:d] = self.predict(pass_inputs, pass_external[i]) # Obtain prediction
                else:
                    predictions[i,:d] = self.predict(pass_inputs, np.array([]))
                # Compute error
                error = np.sqrt(np.mean((validation_data[i]-predictions[i,:d])**2))
                errors[i] = error
        ...
        if plot:
            ...
            plt.plot((np.arange(max_plot_time)+1)*self.timestep, plot_validation[:max_plot_time])
            # plt.xlabel('Time')
            plt.ylabel('x(t)')
            plt.xlim(0,8)
            # plt.ylim(-25,25)
...
class double_reservoir:
    def __init__(self, input_size, approx_num_nodes, input_weight = [1,1], \
                ...
                forget = [0.99,0.99], LM_regularization = [1e-10,1e-10], \
                regularization = [1e-6,1e-6], delta = [1e6,1e6], timestep = 0.05, \
                LM_t_regularization = [1e-6,1e-6], t_regularization = [1e-6,1e-6], \
                t_weighted = False, RLS_reg_type = 'LM', Win=[np.nan,np.nan], A = [np.nan,np.nan], seed = 1):
        # Initializes the reservoir
        self.res1 = reservoir(input_size, approx_num_nodes, input_weight[0], \
                spectral_radius[0], leakage[0], average_degree[0], \
                forget[0], LM_regularization[0], \
                regularization[0], delta[0], timestep, \
        ...
        self.res2 = reservoir(input_size, approx_num_nodes, input_weight[1], \
                ...
                regularization[1], delta[1], timestep, \
    ...
    def valid_time(self, validation_data, error_bound = 0.2, plot = False):
        ...
        predictions = np.zeros(validation_data.shape)
        errors = np.zeros(validation_data.shape[0])
        initial_r_1 = np.copy(self.res1.r)
        initial_r_2 = np.copy(self.res2.r)
        if not self.t_weighted:
            for i in range(validation_data.shape[0]):
                out1, out2 = self.predict() # Obtain prediction
                preditions[i] = out1 + out2
                # Compute error
            ...
            time = np.copy(self.timestep)
            for i in range(validation_data.shape[0]):
                out1, out2 = self.predict_tweighted(time) # Obtain prediction
                preditions[i] = out1 + out2
                # Compute error
                error = np.sqrt(np.mean((validation_data[i]-predictions[i])**2))
                errors[i] = error
...
def vt_min_function_norm_external(data, external_data, hyperparams, mask, base_Win, base_A, num_nodes = 210, \
    ...
    for i in range(len(mask)):
        if mask[i] == 'input_weight':
            ...
            raise ValueError
    if len(data.shape) == 2:
        res = reservoir(data.shape[1]+1, num_nodes, Win = base_Win, A = base_A, forget = forget, \
            input_weight = input_weight, spectral_radius = spectral_radius, \
            regularization = regularization, leakage = leakage)
    else:
        res = reservoir(1, num_nodes, Win = base_Win, A = base_A, forget = forget, \
    ...
    if progress:
        with tqdm(total = num_tests) as pbar:
            for i in range(num_tests):
                ...
                # Get validation data
                if external_input:
                    resync_data     = np.hstack((data[split_pred_starts[i]:split_pred_starts[i]+sync_length], \
                                                 external_data[split_pred_starts[i]:split_pred_starts[i]+sync_length]))
                else:
                    resync_data     = data[split_pred_starts[i]:split_pred_starts[i]+sync_length]
                validation_data = data[split_pred_starts[i]+sync_length:split_pred_starts[i]+sync_length+pred_length]
                external_data_test = external_data[split_pred_starts[i]+sync_length:split_pred_starts[i]+sync_length+pred_length]
                # Train the reservoir
                if train_method == 'RLS':
                    reservoir.trainWoutRLS(train_data, sync_length, external_data_train, external_input = external_input, \
                                          external_output = external_output)
                else:
                    reservoir.trainWout(train_data, sync_length, external_data_train, external_input = external_input, \
                                          external_output = external_output)
                # Evaluate the prediction
                reservoir.synchronize(resync_data)
